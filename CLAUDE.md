...
### 실행 목적
매일 Steam/Epic/Nintendo 게임 할인 정보를 수집하고, Notion 페이지를 최신 정보로 교체한다.
...
#### 4. Epic Games 할인
- CheapShark API: `https://www.cheapshark.com/api/1.0/deals?storeID=25&upperPrice=15&sortBy=Metacritic&pageSize=30`
  - storeID=25: Epic Games Store
  - upperPrice=15: $15 이하 (한화 약 15,000원 미만에 근사)
  - sortBy=Metacritic: 메타크리틱 점수 내림차순
  - pageSize=30: 상위 30개
- 할인 중인 게임만 (`savings` > 0)

#### 5. Nintendo eShop 관심 게임 할인
- `config.json`의 `nintendo.wishlist`에서 전체 NSUID 목록 추출
- `nintendo.price_api`에 `&ids=NSUID1,NSUID2,...` (콤마 구분, 한 번에 전체 조회)
- 응답에서 `discount_price` 필드가 있는 게임만 할인 중
- 필터: `discount_price.raw_value` ≤ `1500000` (15,000원 이하, 100으로 나누지 않음)
- 할인 없으면 "현재 할인 중인 관심 게임이 없습니다." 표시
- 각 게임명에 `https://store.nintendo.co.kr/{nsuid}` 링크 포함
- tag 필드(공통/해원/소담)도 표시
...
## Nintendo eShop 관심 게임

| 게임명 | 정가 | 할인가 | 할인율 | 대상 |
| --- | --- | --- | --- | --- |
| [게임명](스토어링크) | ₩XX,XXX | ₩XX,XXX | XX% | 해원 |

(할인 없으면 "현재 할인 중인 관심 게임이 없습니다. (등록: X개)" 표시)
```
